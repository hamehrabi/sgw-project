...
| `StalenessBanner` | State that the picture is old, and how old. | last good load time, failing file name | hidden, visible | Appears whenever the last good picture is being shown instead of current data, and names the file that failed (REQ-NF-003). It is not dismissible — the whole point is that it outlasts the reader's attention. |
