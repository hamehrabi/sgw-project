...
| CON-006 | Budget | **Amended by ADR-009, and again by CHG-041.** Two paid third-party services: a hosted OpenAI model that phrases computed reasons and drafts the situation summary (ADR-009, CHG-040), and the Google Maps JavaScript API drawing the asset map (CHG-041) — the second taken as its own decision, exactly as this row required, by the client on 2026-08-16. The original constraint — *no paid third-party services in version one* — held from Round 2 until ADR-009 and is quoted here because a reversed constraint should keep its own words. **A third paid service is a new decision, not an extension of these two.** The map degrades to a local SVG scatter with no key configured, so the service is on neither the critical path nor the render path of any other screen. |
