...
| STEST-005 | SEC-Z-002, REQ-R-001 | Weak authorization | A `user` role calls the scenario upload endpoint **directly**, bypassing the interface. | 403; no scenario row; **no file written to disk**; a refusal row appended to the decision record. | Planned |
