...
| FTEST-002 | REQ-NF-003 | A prepared data file becomes unreadable after load | Remove or corrupt one file while the scenario is in use | Every screen renders the last good picture; **no blank screen, no error page**; the failing file is named | `SCENARIO_DATA_UNREADABLE` | Planned |
